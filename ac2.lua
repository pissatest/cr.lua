--[[
    Script: Auto Deploy for a new game (V2)
...
-- Configuration
-- -------------------------
-- Teleport coordinates for Safe Mode, updated with your values
...
local function getCurrentElixir()
    -- Updated with the correct paths for elixir values
...
RunService.Heartbeat:Connect(function()
    ...
    -- Loop through all units to find BOTH King Towers
...
local function attemptToDeployUnit(unitName)
    ...
    local deployTile = teamTiles and teamTiles:FindFirstChild("Tile") 
...
local function autoDeployLoop()
    if (os.clock() - lastDeployTime) < 1 then return end
...
local function safeModeCheck()
    ...
    -- Check for towers by their position, since they share the same name
    for _, model in ipairs(unitsFolder:GetChildren()) do
...
Fluent:Notify({
    ...
    Content = "Coordinates and elixir paths are now up to date.",
    Duration = 8
})
